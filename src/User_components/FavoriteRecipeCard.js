...
export default function FavoriteRecipeCard(props) {
    ...
    const handleDeleteRecipe = async () => {
        ...
        const response = await fetch(`${process.env.REACT_APP_BASE_URL}/user/deleteFavouriteRecipe`,{
            ...
            body:JSON.stringify({userId:user._id, recipeId:props.data._id})
  ...
  return (
    <div className='recipe-box'>
            ...
            <div className='recipe-card-body'>
                <Link to={`/view-recipe/${props?.data?._id}`} className='text-dark mb-2 fw-600 fs-5 text-decoration-none d-block'>{props?.data?.title}</Link>
                ...
                </svg>{props?.data?.ingredients.length} ingredients</p>
            </div>
            <hr />
            <div className='recipe-card-footer'>
            
            <ButtonGroup size="sm">
                <Button className='bg-none border-0 text-custom-grey fs-15 py-0 me-2'><svg className='me-2' xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                ...
                <Button className='bg-none border-0 text-danger fs-15 py-0' onClick={handleDeleteRecipe}>Delete</Button>
            </ButtonGroup>
