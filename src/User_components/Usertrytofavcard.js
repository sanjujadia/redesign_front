...
export default function Usertrytofavcard(props) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (data.status && data.data.length > 0) {
          // Assuming you want the recipe ID from the first item in the array
          setRecipeId(data.data[0].recipeId);
        } else {
  ...
  return (
    <div className='recipe-box'>
            <div className='recipe-card-img'>
                <img src={props?.data?.photo} className='w-100 h-100 object-fit-cover' />
            </div>
            <div className='recipe-card-body'>
            <Link to={`/view-recipe/${recipeId}`}  className='text-dark mb-2 fw-600 fs-5 text-decoration-none d-block'>{props?.data?.name}</Link>
                <p className='text-custom-grey fw-normal fs-17  mb-0 d-inline me-3'><svg className='me-2' xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                ...
                </svg>{props?.data?.totaltime} minutes</p>
                <p className='text-custom-grey fw-normal fs-17 d-inline '>
