...
export default function CreateUserRecipe() {
    ...
    const handleUpdateRecipe = async () => {
        if (recipe.title && recipe.time && recipe.servingSize && recipe.directions.length > 0 && recipe.ingredients.length > 0 && recipe.notes.length > 0) {
            if (window.location.pathname === '/create-user-recipe') {
                ...
                formData.append('recipe', JSON.stringify({ ...recipe, userId}));
              
        
                const response = await fetch(`${process.env.REACT_APP_BASE_URL}/user/createaRecipe`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ ...recipe, userId, image })
                    // body: formData,
                }).then(res => res.json()).then(data => { return data })
                console.log(response,"response");
                if (response.status) {
                    console.log(response,"response_user_form");
            ...
            <div className='mt-1 main-content'>
                <div className='bg-white py-3 px-4'>
                    <Row>
                        <Col mg={8}>
                            <div>
                                <Link to={"/my-recipe"} className='text-dark mb-0 fw-600 fs-5 text-decoration-none'>
                        ...
                        <Col md={4}>
                            <div className='text-end'>
                                <Button className='text-white mb-0 fw-600 fs-17 bg-green custom-shadow border-0 px-3' onClick={() => handleUpdateRecipe()}>{window.location.pathname === '/create-user-recipe' ? 'Add recipe' : 'Save Recipe'}</Button>
                            </div>
                        </Col>
                    </Row>
                            ...
                            <div className='bg-white shadow rounded h-xl-auto position-relative'>
                                ...
                                <div className='px-4 '>
                                    <div className='tag-selected'>
                                        ...
                                        {window.location.pathname === '/create-user-recipe' ?
