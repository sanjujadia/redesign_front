...
export default function ViewMealPlan() {
    ...
    const fetchMealPlan = async () => {
        const res = await fetch(`${process.env.REACT_APP_BASE_URL}/user/getSavedMealPlan/${mealPlanId}`, {
            method: 'GET',
            headers: {
    ...
    const handleDeleteMealPlan = async () => {
        const res = await fetch(`${process.env.REACT_APP_BASE_URL}/user/deleteSavedMealPlan/${mealPlanId}`, {
            method: 'DELETE'
        }).then(res => res.json())
                    ...
                    <Row>
                        ...
                        <Col lg={8}>
                            <div className='text-end'>
                                <Button className='bg-none border-0 text-custom-grey fs-15 py-0' onClick={() => navigate(`/add-userplan/${mealPlanId}`)}><svg className='me-2' xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
